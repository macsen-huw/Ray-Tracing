...
void RenderWidget::paintGL()
    ...
    for (unsigned int i = 0; i < renderParameters->lights.size(); i++)
    {
        ...
        float lightColour[4] = {c.x, c.y, c.z, c.w};

        glLightfv(GL_LIGHT0+i, GL_POSITION, lightPosition);

        glLightfv(GL_LIGHT0+i, GL_AMBIENT, lightColour);
        glLightfv(GL_LIGHT0+i, GL_DIFFUSE, lightColour);
        glLightfv(GL_LIGHT0+i, GL_SPECULAR, lightColour);
